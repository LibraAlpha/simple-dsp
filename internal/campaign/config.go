...
type ConfigManager struct {
    configs map[string]*CampaignConfig // 计划配置映射
    mu      sync.RWMutex              // 读写锁
}
...
func NewConfigManager() *ConfigManager {
    return &ConfigManager{
        configs: make(map[string]*CampaignConfig),
    }
}

// SetConfig 设置计划配置
func (m *ConfigManager) SetConfig(config *CampaignConfig) error {
...
// GetConfig 获取计划配置
func (m *ConfigManager) GetConfig(campaignID string) (*CampaignConfig, bool) {
    m.mu.RLock()
    defer m.mu.RUnlock()
...
func (m *ConfigManager) ListConfigs() []*CampaignConfig {
    m.mu.RLock()
    defer m.mu.RUnlock()
    
    configs := make([]*CampaignConfig, 0, len(m.configs))
