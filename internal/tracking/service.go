...
func (s *Service) Track(ctx context.Context, event *TrackingEvent) error {
    startTime := time.Now()
    defer func() {
        s.metrics.TrackingDuration.WithLabelValues(string(event.EventType)).Observe(time.Since(startTime).Seconds())
    }()
    ...
    for i := 0; i <= trackingConfig.RetryCount; i++ {
        ...
        if resp.StatusCode >= 200 && resp.StatusCode < 300 {
            s.metrics.TrackingSuccess.WithLabelValues(string(event.EventType)).Inc()
            return nil
        }
    ...
    s.metrics.TrackingFailure.WithLabelValues(string(event.EventType)).Inc()
    return lastErr
}
